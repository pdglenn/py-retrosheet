...
def parse_rosters(file, conn, bound_param):
    ...
    for row in reader:
        ...
        sql = "INSERT INTO rosters VALUES (?)" % ", ".join([bound_param] * len(row))
...
def parse_teams(file, conn, bound_param):
    ...
    for row in reader:
        ...
        sql = "INSERT INTO teams VALUES (?)" % ", ".join([bound_param] * len(row))
        conn.execute(sql, row)
...
def parse_games(file, conn, bound_param):
    ...
    if conn.engine.driver == 'psycopg2':
        ...
        conn.execute('COPY games FROM ? WITH CSV HEADER', file)
    else:
        reader = csv.reader(open(file))
        headers = reader.next()
        for row in reader:
            ...
            sql = 'INSERT INTO games(?) VALUES(?)' % (','.join(headers), ','.join([bound_param] * len(headers)))
            conn.execute(sql, row)
